        ...
        <div className="flex items-center justify-between gap-2 h-16 min-w-0">
          ...
          <div className="flex items-center gap-1 sm:gap-2 shrink-0 ml-auto">
            ...
            {(user?.role === 'agent' ||
              ...
              <button
                type="button"
                onClick={() => handleProtectedAction(openAddPropertyModal)}
            ...
            {handleOpenMessages && (
              <button
                type="button"
                onClick={() => handleProtectedAction(handleOpenMessages)}
                ...
                title={user?.role === 'client' ? 'Mes Messages & Demandes' : 'Messagerie'}
              >
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center space-x-3.5 pb-3 border-b border-slate-100 dark:border-[#2e2e2e]">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <span className="inline-block mt-0.5 text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300">
                            {user.role}
                          </span>
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        className="w-full py-2.5 px-3 bg-rose-50 hover:bg-rose-100 dark:bg-rose-950/40 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-900/50 rounded-xl text-xs font-bold transition flex items-center justify-center space-x-2 cursor-pointer shadow-xs"
                      >
                        <LogOut className="w-4 h-4" />
                        <span>{language === 'en' ? 'Sign Out' : language === 'sw' ? 'Toka' : 'Se Déconnecter'}</span>
                      </button>
                    </div>
                  ) : (
                    ...
                    <div className="space-y-3">
                      <div className="text-center py-2">
                        ...
                        <h4 className="text-xs font-black text-slate-900 dark:text-white">
                          {language === 'en' ? 'Guest Account' : language === 'sw' ? 'Akaunti ya Mgeni' : 'Compte Invité'}
                        </h4>
                        <p className="text-[11px] text-slate-600 dark:text-slate-400 mt-0.5">
                          {language === 'en'
                            ? 'Connect to manage listings and save favorites.'
                            : language === 'sw'
                            ? 'Ingia ili udhibiti matangazo na uhifadhi vipendwa.'
