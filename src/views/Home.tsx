...
export const Home: React.FC<HomeProps> = ({
  ...
  const displayedProperties = useMemo(() => {
    let list = properties.filter(isPropertyActiveAndVisible);
      ...
      >
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-5 sm:px-8 py-10 sm:py-14 w-full text-center space-y-3">
          <h1 className="text-2xl sm:text-4xl lg:text-5xl font-black tracking-tight text-white leading-tight">
            {t('heroTitlePrefix', language === 'sw' ? 'Pata nyumba yako huko ' : 'Trouvez votre bien à ')}<span className="text-[#FF385C]">Bukavu</span>
          </h1>
          <p className="text-xs sm:text-base text-slate-200 font-medium max-w-lg mx-auto leading-relaxed">
            {t('heroSubtitle', language === 'sw' ? 'Nyumba za kupangisha, zinazouzwa na viwanja huko Ibanda, Kadutu na Bagira.' : 'Locations résidentielles, ventes et parcelles à Ibanda, Kadutu et Bagira.')}
          </p>
        </div>
      </section>
      ...
      <section id="search-section" className="max-w-4xl mx-auto px-4 sm:px-6 w-full -mt-4 sm:-mt-6 relative z-20">
        <div className="bg-white dark:bg-[#1a1a1a] p-2.5 sm:p-3 rounded-2xl sm:rounded-full shadow-2xl border border-slate-200 dark:border-white/10 flex flex-col sm:flex-row items-center gap-2.5">
          ...
          <div className="flex-1 w-full flex items-center space-x-2.5 px-3.5 py-2">
            ...
            <input
              type="text"
              placeholder={language === 'sw' ? 'Jirani, mtaa au neno la kutafuta...' : language === 'en' ? 'Neighborhood, street or keyword...' : 'Quartier, rue ou mot-clé (ex: Nguba, Muhungu, Labotte)...'}
              value={quickQuery}
          ...
          <div className="w-full sm:w-auto px-3.5 py-1 flex items-center space-x-2">
            ...
            >
              <option value="tous" className="bg-white dark:bg-[#242424] text-slate-900 dark:text-white">{language === 'sw' ? 'Manispaa zote' : language === 'en' ? 'All communes' : 'Toutes communes'}</option>
          ...
          >
            ...
            <span>{t('searchButton', language === 'sw' ? 'Tafuta Nyumba' : 'Rechercher')}</span>
          </button>
        </div>
      </section>
      ...
      <section id="featured-listings" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6 pt-4 sm:pt-6">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            <h2 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">
              {language === 'sw' ? 'Fursa za hivi karibuni' : language === 'en' ? 'Latest opportunities' : 'Dernières opportunités'}
            </h2>
          </div>
          ...
          <div className="flex items-center space-x-1 overflow-x-auto no-scrollbar bg-slate-100 dark:bg-white/5 p-1 rounded-xl">
            {(
              [
                { id: 'tous', label: language === 'sw' ? 'Zote' : language === 'en' ? 'All' : 'Tous' },
                { id: 'location', label: language === 'sw' ? 'Upangishaji' : language === 'en' ? 'Rentals' : 'Locations' },
                { id: 'vente', label: language === 'sw' ? 'Mauzo' : language === 'en' ? 'Sales' : 'Ventes' },
                { id: 'parcelle', label: language === 'sw' ? 'Viwanja' : language === 'en' ? 'Plots' : 'Parcelles' },
                { id: 'villa', label: language === 'sw' ? 'Vila' : language === 'en' ? 'Villas' : 'Villas' }
              ] as const
            ).map((tab) => (
              <button
                key={tab.id}
        ...
        {properties.length === 0 ? (
          <div className="bg-white dark:bg-[#1a1a1a] rounded-3xl p-12 text-center border border-slate-200/80 dark:border-white/5 space-y-4">
            ...
            <div>
              <p className="text-sm font-bold text-slate-900 dark:text-white">{language === 'sw' ? 'Hakuna nyumba inayopatikana' : language === 'en' ? 'No properties available' : 'Aucune propriété disponible'}</p>
              <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5">{language === 'sw' ? 'Kuwa wa kwanza kuongeza tangazo.' : language === 'en' ? 'Be the first to add a listing.' : 'Soyez le premier à ajouter une annonce.'}</p>
            </div>
            <button
              type="button"
            ...
            >
              {language === 'sw' ? 'Chapisha nyumba' : language === 'en' ? 'Post a property' : 'Publier un bien'}
            </button>
          </div>
        ) : displayedProperties.length === 0 ? (
          <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl p-8 text-center border border-slate-200/80 dark:border-white/5">
            <p className="text-xs font-medium text-slate-600 dark:text-slate-400">{language === 'sw' ? 'Hakuna nyumba kwa kichujio hiki.' : language === 'en' ? 'No property matches this filter.' : 'Aucun bien pour ce filtre.'}</p>
            <button
              type="button"
              onClick={() => setGridFilter('tous')}
              className="text-xs font-semibold text-[#FF385C] mt-2 hover:underline cursor-pointer"
            >
              {language === 'sw' ? 'Onyesha yote' : language === 'en' ? 'Show all' : 'Afficher tout'}
            </button>
          </div>
        ) : (
        ...
        {properties.length > 6 && (
          <div className="text-center pt-4">
            ...
            >
              <span>{language === 'sw' ? `Tazama orodha yote (${properties.length} matangazo)` : language === 'en' ? `View full catalog (${properties.length} listings)` : `Voir tout le catalogue (${properties.length} annonces)`}</span>
